...
class Auth extends CI_Controller
{
	...
	*/
	...
	// Added with the facebook auth, Mar 3, 2015 SW
	function facebook()
	{
	...
	// 
	function facebookresponse($type = null) 
	{
		// Facebook handler for login
		if (isset($_GET['code'])) {
			$this->facebook_ion_auth->login();
			//if (!$this->ion_auth->logged_in() || !$this->ion_auth->is_admin()) {
			if (!$this->ion_auth->logged_in()) { // removed is_admin because it doesnt make sense.
				header('Location:/#fbauth=failblog');
				exit();
			}
			header('Location:/#welcomeback');
		} else {
			show_error('Well, I guess facebook sign in didn\'t really work out...');
		}
	}

	// Added with the saml/onelogin auth, June 9, 2018 SW
	// simple login
	...
	function saml_login()
	{
		...
		$this->load->library('saml_auth');
	    $this->saml_auth->login($return);
	}

	// integrated login
	function saml_finishlogin()
	{
		if (isset($_GET["return"])) {
			$return = $_GET["return"];
		} else {
		...
		$this->load->library('saml_auth');
	    $this->saml_auth->finishlogin($return);
	}
