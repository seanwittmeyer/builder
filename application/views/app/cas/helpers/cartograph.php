<?php if (!isset($path)) $path = '/api/visualization/conversation/6'; ?><div class="row">
	<div class="col-lg-12" id="graphcontainer" style="
...
function drawcartograph() {(function() {
	drawcartograph = null;
	var a = $("#graphcontainer").width(),
		c = a*.7,
		h = c,
		U = 130, // principle rect width
		KK = 35, // principle rect height
		K = 5, // maybe spacing of principles
		S = 15, // midpoint of connecting arc between edge circle and principle rect
		s = 8, // multiplier of arc above
		R = 110, // spacing multiplier of side elements
		J = 30, // offset arcs of terms on left/right from centerline of diagram
		o = 22, // vertical text offset
		t = 10, // side dot-text distance
		w = 1000,
		F = "elastic",
		N = "#2780e3";
	var T, q, x, j, H, A, P;
	...
	d3.json("<?php print($path); ?>", function(X, Y) {
	...
	function O() {
		...
		A.forEach(function(af, ag) {
			...
			a2 = (ag[1].x - 90) * Math.PI / 180, bulge = 40;
			P.push({
	...
	function D(Y) {
		...
		X.append("rect").attr("x", U / -2).attr("y", K / -2).attr("width", U).attr("height", KK).transition().duration(w).ease(F).attr("x", function(Z) {
			return Z.x
		}).attr("y", function(Z) {
			return Z.y
		});
		X.append("text").attr("x", 0).attr("y", function(Z) {
			return K / -2 + o
		}).attr("fill", "#fff").attr("text-anchor", "middle").text(function(Z) {
			return Z.name
		}).transition().duration(w).ease(F).attr("x", 0).attr("y", function(Z) {
			return Z.y + o
		});
...
<?php if (isset($onload) && $onload == true) { ?>drawcartograph();
<?php } ?>
</script>
