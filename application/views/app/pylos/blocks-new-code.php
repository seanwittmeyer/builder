		...
		<div class="row">
			<div class="col-lg-11">
				...
				<form id="formnewblock" class="form-horizontal" enctype="multipart/form-data">
					...
					<p>You made something and want to share it, now let's get a good title so people can find your resource. <strong>If your block is file based</strong>, <a href="<?php echo site_url('pylos/blocks/create'); ?>">try creating it here instead</a>.</p>
					...
					<div class="form-group">
						<label for="payload[code]" class="col-md-3 control-label">Place your code here, you can set the language below. We default to python.</label>
						<div class="col-md-9 imageset"><textarea type="text" id="codeeditor" name="payload[codefield]" placeholder="Enter your code/script here..."></textarea></div>
						<input id="pylos-block-code" type="hidden" name="payload[code]" value="" />
					...
					<div class="form-group">
						<div class="col-sm-offset-2 col-md-9">
							<div class="checkbox">
								<label>
									<input type="checkbox" id="pylos_newblock_private" name="payload[private]"> Limit access to this resource
								</label>
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-md-9"><input type="text" class="form-control" name="payload[tags]" id="pylos_newblock_tags" placeholder="analysis, energyplus, etc..."><span class="small">If you don't see the tags you want in the list above, you can add them here. Separate by commas.</span></div>
					</div>
					<div class="form-group">
						<label for="payload[dependencies]" class="col-md-3 control-label">Dependencies</label>
						<div class="col-md-9"><input type="text" class="form-control" name="payload[dependencies]" id="pylos_newblock_dependencies" placeholder="diva, honeybee, human ui, etc..."><span class="small">If you don't see the tags you want in the list above, you can add them here. Separate by commas.</span></div>
					</div>
					<div class="form-group">
						<label for="payload[project]" class="col-md-3 control-label">Project Number from Vision</label>
						<div class="col-md-9"><input type="text" class="form-control" name="payload[project]" id="pylos_newblock_project" placeholder="P23796.00"><span class="small">Adding a project helps folks learn how certain things were done if they know a project. Leave this blank if the resource was intended to be generic.</span></div>
					</div>
...
<!-- End Content Area -->
	...
	<script>
		...
		$('#pylos_newblock_strategies').selectize({
			...
			createOnBlur: true,
			create: true,
			options: <?php $taglist = array(); asort($strategies); foreach ($strategies as $t) $taglist[] = array('value'=>$t['slug'],'text'=>$t['title']); echo json_encode($taglist); ?>
		});
		...
		$('#submitnewblock').click(function() {
			$('#pylos-block-code').val(editor.getValue());
			$.ajax({
				type: "POST",
				beforeSend: function() {
